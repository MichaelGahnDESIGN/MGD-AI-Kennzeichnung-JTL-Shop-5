...
The public repository does not provide an automatic updater. Install the verified `MGD_AI_Kennzeichnung-1.3.8.zip` through JTL's manual plugin upload after a backup and a development-site test. When update notices are enabled, GitHub can technically receive the server IP, time and fixed User-Agent; no images, tokens, customer or form data are sent, and positive or negative results are cached for twelve hours.
