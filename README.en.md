...
Version 1.3.4 keeps the two large, stacked AI-philosophy editors from Version 1.3.0, clears JTL's backend template cache through the actually active Smarty engine after an update, and starts the fully local editor reliably inside JTL's AJAX-loaded plugin tab. Authors can work in **Visual** mode or switch to the optional **HTML** source view. The local toolbar supports paragraphs, level-two and level-three headings, lists, strong and emphasised text, and safe HTTPS links. **Save both language versions** submits the German and English content together.
...
The public repository does not provide an automatic updater. Install the verified `MGD_AI_Kennzeichnung-1.3.4.zip` through JTL's manual plugin upload after a backup and a development-site test. When update notices are enabled, GitHub can technically receive the server IP, time and fixed User-Agent; no images, tokens, customer or form data are sent, and positive or negative results are cached for twelve hours.
