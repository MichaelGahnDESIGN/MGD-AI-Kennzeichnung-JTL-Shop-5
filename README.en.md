...
Version 1.3.0 adds two large, stacked language editors for the AI philosophy. Authors can work in **Visual** mode or switch to the optional **HTML** source view. The local toolbar supports paragraphs, level-two and level-three headings, lists, strong and emphasised text, and safe HTTPS links. **Save both language versions** submits the German and English content together.
...
The public repository does not provide an automatic updater. Install the verified `MGD_AI_Kennzeichnung-1.3.0.zip` through JTL's manual plugin upload after a backup and a development-site test. When update notices are enabled, GitHub can technically receive the server IP, time and fixed User-Agent; no images, tokens, customer or form data are sent, and positive or negative results are cached for twelve hours.
