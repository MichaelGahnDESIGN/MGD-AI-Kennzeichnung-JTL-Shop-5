...
Version 1.3.9 restores the neutral shoe preview and adds a separate, locally rendered 2× detail preview on colourful shapes and fine lines. Both previews share the current label settings, making transparency and background blur easier to compare. Recursive OPC uploads, the design-save fix and compact editor controls remain included. Existing labels and gallery filters are preserved. Release publication and shop installation are separate steps; see the release notes for the actual verification and deployment status. Test updates on a backed-up development shop before using them in production.
...
The public repository does not provide an automatic updater. Install the verified `MGD_AI_Kennzeichnung-1.3.9.zip` through JTL's manual plugin upload after a backup and a development-site test. When update notices are enabled, GitHub can technically receive the server IP, time and fixed User-Agent; no images, tokens, customer or form data are sent, and positive or negative results are cached for twelve hours.
