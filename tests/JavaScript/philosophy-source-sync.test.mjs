...
test('zwei Sprachinstanzen teilen keinen Zustand', () => {
    const deutsch = erstelleInstanz({ source: '<p>Deutsch</p>' });
    const englisch = erstelleInstanz({ source: '<p>English</p>' });
...
test('Sanitizer- und Adapterfehler schreiben keinen unsicheren Rohwert in das Formular', () => {
    const sanitizerFehler = erstelleInstanz({ source: '<script>unsicher()</script>' }, {
        sanitize() {
    ...
    const renderFehler = erstelleInstanz({ source: '<p>Alt</p>' }, {
        visual: {
            render() {
    ...
    renderFehler.synchronisierung.showHtml();
    renderFehler.htmlFeld.value = '<p>Sicher</p><script>unsicher()</script>';
...
test('ungültige Sanitizer-Rückgaben und Moduswechsel werden fail-closed behandelt', () => {
    ...
    assert.deepEqual(synchronisierung.showVisual(), { ok: false, mode: 'visual', value: '' });
    assert.deepEqual(synchronisierung.showHtml(), { ok: false, mode: 'html', value: '' });
    assert.deepEqual(modeAenderungen, ['visual', 'html']);
    assert.equal(sourceFeld.value, '');
    assert.equal(htmlFeld.value, '');
    assert.equal(visualAdapter.gerendert, '');
});
